...
<title>Modifier Assiste</title>
</head>
<body>
<%@include file="menu.jsp" %>
<% DaoEleve eleve = new DaoEleve();
   DaoSeance seance = new DaoSeance();
%>
...
<div class="container">
  ...
  <div class="panel panel-info">
    <div class="panel-heading">Modifier Assiste</div>
    ...
    <form role="form" action="Assistectrl?action=edit" method="post" >
   <form class="form-horizontal">
         
          <div class="form-group" style="padding: 20px;">
            <label for="rang" class="control-label col-xs-2">Nb Fautes</label>
            <div class="col-xs-10">
                <input type="text" class="form-control" name="nbfautes" value="${p.nbfautes}">
            </div>
        </div>
        ...
        <div class="form-group" style="padding: 20px;">
		    ...
		    <div class="col-xs-10">
			      <select class="form-control" name="dateaheurea">
			       <c:forEach var="p" items="<%= seance.findAll() %>" > 
			        <option value="${p.id.dates}_${p.id.heures}">${p.id.dates}_${p.id.heures}</option>
			        </c:forEach>
		      	 </select>
		     </div>
      	</div>
